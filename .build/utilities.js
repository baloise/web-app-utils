...
function parseUtility({ filePath, fileContent, multiple, assertedReturnType }) {
  const sourceFile = createSourceFile(fileContent)
  const functionDeclaration = filterFunctionDeclaration(sourceFile.statements)
  const exportedFunctionStatements = filterExportedStatements(functionDeclaration)
  ...
  return exportedFunctionStatements.map(statement =>
    parseFunction({ statement, sourceFile, assertedReturnType, filePath }),
  )
}
